...
import { X, Link as LinkIcon, Check, Image as ImageIconLucide } from 'lucide-react-native';
...
export default function ShareScreen() {
  ...
  const fetchLinkMetadata = async (linkUrl: string) => {
    ...
    try {
      ...
      const validTypes: LinkType[] = ['link', 'video', 'image', 'music', 'text'];
      if (validTypes.includes(detectedType) && detectedType !== 'image') { // Don't auto-switch to image from web URL detection
  ...
  const handleSave = async () => {
    ...
    if (selectedType !== 'text' && !url.trim()) {
      alert(selectedType === 'image' ? 'Please choose an image.' : 'Please enter a URL.');
      return;
    }
    ...
    if (!savvyTitle) {
      ...
      else if (selectedType !== 'text') savvyTitle = url; // Fallback to URL if title empty (not for image)
  ...
  const onTypeSelect = (newType: LinkType) => {
    ...
    if (oldType !== newType) {
      ...
      if (newType !== 'text' && newType !== 'image' && url && !url.startsWith('file://')) {
  ...
  return (
    ...
    >
      <View style={[styles.header, { borderBottomColor: colors.border }]}>
        ...
        <Text style={[styles.headerTitle, { color: colors.text }]}>Save Savvy</Text>
        <TouchableOpacity 
          onPress={handleSave} 
      ...
      <ScrollView contentContainerStyle={styles.content}>
          ...
          <View style={[styles.inputContainer, { backgroundColor: colors.card, borderColor: colors.border }]}>
            ...
            <TextInput
              ...
              placeholder={
                ...
                selectedType === 'music' ? "Music URL" :
                "https://example.com"
              }
              placeholderTextColor={colors.textSecondary}
              value={url}
              onChangeText={(text) => { setUrl(text); setIsMetadataFetched(false); }}
              autoCapitalize="none"
              autoCorrect={false}
              keyboardType="url"
        ...
        <View style={[styles.textAreaContainer, { backgroundColor: colors.card, borderColor: colors.border }]}>
          <TextInput
            ...
            placeholder={
              selectedType === 'text' ? "Start writing your note..." : "Description (optional)"
            }
            placeholderTextColor={colors.textSecondary}
            value={description}
